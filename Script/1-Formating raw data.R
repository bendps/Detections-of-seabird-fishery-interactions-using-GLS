...
library(data.table) ; library(baytrends) ; 
...
for (i in 1:nrow(deployments)){
  ...
  if (length(myact.file) == 0 | length(mylight.file) == 0){
    ...
    myactivity <- paste(folderpath,myact.file, sep="/") %>%
      map_dfr(readRDS)
    mylight <- paste(folderpath,mylight.file, sep="/") %>%
      map_dfr(readRDS)
    ...
    # 1440 = gap bigger than 10 days in data are considered to be polar day/night
