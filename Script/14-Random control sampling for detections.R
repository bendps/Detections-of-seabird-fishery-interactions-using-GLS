...
# Global process
# - for a given detection, get its characteristics
...
#Load detections 
mydetect <- readRDS("Data/Fulmar/Encounters/detect_final_zoned.rds") 
...
mycontrol<- mycontrol %>% filter(is.na(mycontrol$activity_cat)==F)


#Group to compare to the data
grpcontrol <- mycontrol %>% group_by(zone, activity_cat, individ_id) %>% summarise(n = n())
grpcontrol <- grpcontrol %>% ungroup() %>%  filter(is.na(grpcontrol$zone)==F)
...
#Save
saveRDS(grpcontrol, "Data/Fulmar/control_stat_encounter.rds") 
