...
library(cowplot) ; library(data.table) 
library(ggpubr)
#~~~~~~#
...
light.detect <- function(mydatan){
  ...
  while (i <= nrow(mydatan)){ #Scan the data
    if (mydatan$raw_light[i] > threshold){ #Check if light is over the threshold
      ...
      #Calculate mean light and activity of the encounter
  ...
  if(nrow(detect) >= 1){
    ...
    #Changing the date-time into POSIXct and adding bird ID
...
folderpath <- "Data/Fulmar/Processed/Corrected/Night_and_no_twilight" 
mysavingpath <- "Data/Fulmar/Encounters" 

for(pattern in c("-low-")){ #Apply the function to the different loggers group,  c("-low-","-high-","-no-"))
  ...
  x <- which(tot == "no detection") #remove the element of the list with no detections
  tot[x] <- NULL
