...
#~~~~~~#
library(dplyr); library(lubridate); library(ggplot2); library(tidyverse)
...
folderpath <- "Data/Fulmar/Final" #Can be changed to the output path of the 12. script #!!#
...
world <- readOGR("Data/World_shapefile/World_continental_WGS84.shp", "World_continental_WGS84") #Change to the path were your world shapefile is #!!#

savingpath <- "Data/Fulmar/Final/Located" #Change to where you want the output data to be saved #!!#
...
for(i in 1:length(data.files)){
  ...
  loc <- readRDS("Data/Fulmar/Output_GLS_&_IRMA_locations_Fulmarus_glacialis_2020-04-30.rds") #Can be changed to the path of your locations data #!!#
  ...
  world <- spTransform(world, CRS(proj.aeqd))
