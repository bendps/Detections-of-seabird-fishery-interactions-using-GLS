...
# List of files
folderpath <- "Data/Fulmar/Processed" #change to the output path of script 1. #!!#
data.files <- list.files(folderpath, pattern = "DATA")
mysavingpath <- "Data/Fulmar/Processed/Corrected" #change to the path where you want the corrected data to be saved #!!#
...
for (i in 1:length(data.files)){
  ...
  polar <- light2 %>%
    ...
    filter(Count >= 3 & val == 2) %>%      # keep only cases where you have 3+ daylight = 2
    ...
    if(polar$Beg > date(min(myloc$timestamp)) &
      ...
      if(nrow(polar) > 0){
        ...
        if(nrow(myloc) > 0){
          ...
          # Interval that need to be delete for one individual
